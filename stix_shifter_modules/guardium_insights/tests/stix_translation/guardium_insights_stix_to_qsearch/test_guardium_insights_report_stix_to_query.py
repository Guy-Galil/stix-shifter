...
options_file = open('stix_shifter_modules/guardium_insights/tests/stix_translation/guardium_insights_stix_to_qsearch/options.json').read()


category = "\"report_id\": \"000000000000000000000901\""
category1 = "\"report_id\": \"000000000000000000000902\""
translation = stix_translation.StixTranslation()


def _test_query_assertions(query, ind, filters):
    assert query[ind].find(category) >= 0 or query[ind].find(category1)
    assert query[ind].find(filters) >= 0 


def _translate_query(stix_pattern):
    return translation.translate('guardium_insights', 'query', '{}', stix_pattern)
...
class TestQueryTranslator(unittest.TestCase, object):
    ...
    def test_severity_query(self):
        stix_pattern = "[ x-guardium:severity='High']"
        query = _translate_query(stix_pattern)
